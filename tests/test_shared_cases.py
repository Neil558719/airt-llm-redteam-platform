...
def test_shared_cases_preserve_multiturn_and_tool_argument_expectations():
    cases = shared_security_cases(ROOT / "shared_cases" / "unified_chatflow.yaml")
    multi = next(case for case in cases if case.id == "multiturn_injection_001")
    assert len(multi.turns) == 2
    tool = next(case for case in cases if case.id == "tool_argument_tampering_001")
    assert "DEMO-ORD-2026-042" in tool.detect.tool_argument_any
    session = next(case for case in cases if case.id == "session_isolation_001")
    assert len(session.turns) == 2
    assert next(case for case in cases if case.id == "ssrf_internal_url_001").detect.judge_goal
...
def test_shared_cases_support_multimodal_input_metadata():
    cases = load_shared_cases(ROOT / "shared_cases" / "multimodal_chatflow.yaml")
    assert len(cases) == 7
    assert {item.input.type for item in cases} == {"image", "audio"}
    assert all(item.input is not None and item.security is not None for item in cases)
    assert all(item.security.judge_goal for item in cases)
from pathlib import Path
