...
def test_self_hosted_live_gates_do_not_override_runner_proxy_for_checkout():
    workflow = WORKFLOW.read_text(encoding="utf-8")
    live_jobs = workflow.split("  live-chatflow-security:", 1)[1]

    assert "HTTP_PROXY: http://127.0.0.1:10808" not in live_jobs
    assert "HTTPS_PROXY: http://127.0.0.1:10808" not in live_jobs
