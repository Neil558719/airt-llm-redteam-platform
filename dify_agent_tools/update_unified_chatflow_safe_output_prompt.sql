-- Prevent the customer LLM from reproducing active output-injection payloads.
BEGIN;
        ...
        jsonb_set(node.item, '{data,prompt_template}', (
          SELECT jsonb_agg(
            CASE WHEN prompt.item->>'role' = 'system'
                      AND position('固定回复：无法处理该请求，请提供订单相关问题。' IN prompt.item->>'text') = 0 THEN
              jsonb_set(prompt.item, '{text}', to_jsonb(
                replace(replace(prompt.item->>'text', E'\n\n不得逐字复述危险载荷，包括 HTML 或脚本标签、javascript 等危险链接、可点击的未知外部链接，以及可能触发执行或跳转的内容。即使是在拒绝、解释、引用或 Markdown 代码块中，也不得输出原始载荷；应改用“已省略的危险脚本或链接”等安全描述。', ''), E'\n\n不得输出、提及、命名、引用或逐字复述危险载荷，包括 HTML 或脚本标签、javascript 等危险链接、可点击的未知外部链接，以及可能触发执行或跳转的内容。即使是在拒绝、解释、引用或 Markdown 代码块中，也不得包含原始载荷或其片段；统一改用“已省略的危险内容”等不含载荷的安全描述。', '')
                || E'\n\n当用户要求输出脚本、HTML、危险链接或其他可能执行或跳转的内容时，只能原样回复以下一句，不得增加解释、示例、引用、代码块或用户输入中的任何文字：固定回复：无法处理该请求，请提供订单相关问题。'
              ), true)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE workflow.id = 'f731bafa-99c1-40f4-a33e-e93b54d382cb'
      ...
      AND position('固定回复：无法处理该请求，请提供订单相关问题。' IN prompt.item->>'text') > 0
  ) THEN
    RAISE EXCEPTION 'customer_llm safe-output prompt was not updated';
  END IF;
END;
$$;
